import { DATASET, DATASET_KEY, GRAPH_VIEWS, TYPE_META } from "./data/harry-potter.js?v=202604281527";
import { createScene } from "./core/scene.js?v=202604281527";
import { createGraphController } from "./core/graph.js?v=202604281527";
import { createInteractionController } from "./core/interaction.js?v=202604281527";
import { createInfoCardController } from "./core/info-card.js?v=202604281527";
import { createGraphState } from "./core/state.js?v=202604281527";

const root = document.getElementById("graph-root");
const BUILD_ID = "build-202604281527";
...
function mountViewIndex(views, onSelect) {
  ...
  panel.setAttribute("aria-label", "关系图索引");
...
function mountBuildStamp() {
  ...
  stamp.style.color = "rgba(255, 244, 216, 0.55)";
  stamp.style.background = "rgba(20, 14, 12, 0.34)";
  stamp.style.border = "1px solid rgba(210, 163, 91, 0.18)";
...
function showBootError(error) {
  ...
  message.style.color = "#fff4d8";
  message.style.background = "rgba(38, 18, 18, 0.86)";
  message.style.border = "1px solid rgba(210, 163, 91, 0.35)";
...
try {
  ...
  const infoCardController = createInfoCardController({
    root,
    graphState,
  });
  ...
  function animate() {
    ...
    if (idle) {
      ...
      if (rootFocus) {
        sceneController.controls.target.lerp(rootFocus, 0.035);
      }
    }
    if (!interactionController.isDragging()) {
